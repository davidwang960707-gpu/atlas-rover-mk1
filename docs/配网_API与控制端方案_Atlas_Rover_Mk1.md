...
1. 现有 DualEye 程序和 MimiClaw 是一起烧录，还是分开烧录？
...
| 底盘专属参数 | Mk.1 先只需要左右轮 PWM 补偿、默认速度、默认持续时间和超时保护；编码器/IMU/PID 作为后续升级项 |
...
| 入口 | 角色 | 高频用户 |
|---|---|---|
| `/app` 或 `/` | 机器人应用控制页 | 日常使用者 |
| `/admin` | 设备管理后台 | 配网、调试和维护者 |
...
- 切换时钟、闹钟、照片、状态页。
- 手动模式下控制小车短时移动。
- 触发 MimiClaw 应用动作，例如听音乐、讲故事、陪聊、设置闹钟。
...
| 页面 | 手机端 | 电脑端 |
|---|---|---|
| 应用首页 | 表情、显示、移动、MimiClaw 应用入口 | 同手机，后续可加键盘控制 |
