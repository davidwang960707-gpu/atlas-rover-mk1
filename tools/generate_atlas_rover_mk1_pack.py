...
BOM = [
    ...
    },
    {
        ...
        "qty": "2",
        "required": "必买",
        "notes": "100 RPM 更适合桌面慢速巡游；150 RPM 可用 PWM 限速。Mk.1 不需要编码器版本，除非你要做里程计。",
        "est_cny": "15-25",
    },
    {
        "class": "运动/驱动",
        ...
        "qty": "1",
        "required": "必买",
        "notes": "这就是解决“ESP32 不能直接带电机”的必备板子。ESP32/PCA9685 只给控制信号，DRV8833 才负责给 N20 电机输出电流。VM 接电机 5 V 升压支路，逻辑电平接 3.3 V，必须共地。",
        "est_cny": "5-10",
    },
    {
        "class": "控制/扩展",
    ...
    },
    {
        ...
        "qty": "2",
        "required": "必买",
        "notes": "43 mm 轮也能用，但要抬高前保险杠，并且速度和外宽都会增加。",
        "est_cny": "6-12",
    },
    {
        "class": "运动/备选",
        ...
        "qty": "2",
        "required": "备选",
        "notes": "仅在买不到合适 N20 橡胶轮时使用。钢齿轮不能默认直接套 N20，必须确认孔径/孔型；外圈建议套 O 型圈或热缩胶圈增加摩擦。",
        "est_cny": "8-20",
    },
    {
        "class": "运动/备选",
        ...
        "qty": "2",
        "required": "备选",
        "notes": "64T 齿轮若不是 3 mm D 孔直连，必须用轮毂/夹片转接；不建议只用胶水粘。",
        "est_cny": "8-18",
    },
    {
        "class": "运动/备选",
        ...
        "qty": "2-4",
        "required": "备选",
        "notes": "钢齿轮直接接触桌面会滑、响、伤桌面；外圈必须加橡胶圈或热缩圈。",
        "est_cny": "3-10",
    },
    {
        "class": "运动",
        ...
        "qty": "1",
        "required": "必买",
        "notes": "前置万向轮能让第一版结构更简单。若底盘不水平，可加 1-2 mm 垫片。",
        "est_cny": "3-8",
    },
    {
        "class": "电源",
        "item": "18650 锂电池",
        "spec": "1S，标称 3.7 V，2600-3500 mAh，建议持续放电 >= 3 A，优先带保护板电芯",
        "qty": "1",
        "required": "必买",
    ...
    },
    {
        "class": "电源",
        "item": "5 V 升压模块",
        "spec": "1S 锂电输入，稳压 5 V 输出，建议 2 A 连续 / 3 A 峰值能力",
        "qty": "1",
        "required": "必买",
        "notes": "只给电机支路使用。不要让电机电流经过 ESP32 的 3.3 V 电源轨。若两个 N20 堵转明显，优先换更大电流模块。",
        "est_cny": "5-12",
    },
    {
        "class": "电源",
...
def write_docs() -> None:
    ...
    write_text(
        ...
        目标是一台可展示、可调试、可继续扩展的小型原型车：黄铜车架、两轮差速、单节 18650 供电、双眼表情显示、板载音频，以及推荐外接 RGB 状态灯。
...
def build_pdf() -> None:
    ...
    story += [
        ...
        p("4. 复审后补强电机供电余量和接线可实施性：18650 建议持续放电 >= 3 A，升压模块建议 2 A 连续 / 3 A 峰值，并准备 SH1.0/FPC 转接线。"),
    ...
    story += [
        p("组装模拟与效果确认", "AtlasH1"),
        p("按当前 V1.0 物料和结构方案，可以做出接近参考图的三层黄铜笼架效果：底层电池和动力、中层驱动/扩展电子、上层双眼显示与交互。真实手工版会看到焊点、线束和模块孔位差异；Mk.1 是两轮差速加前万向轮，不做四驱。"),
        table(
    ...
    story += [
        p("黄铜下料施工单", "AtlasH1"),
        p("参考图用于确定黄铜笼架、前保险杠、双眼护圈、顶部提手、前灯条和侧面轮拱的造型。Mk.1 仍保持两轮差速加前万向轮。"),
        table(
