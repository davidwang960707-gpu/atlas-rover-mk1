...
BOM = [
    ...
    {
        "class": "控制/底盘",
        "item": "Seeed Studio XIAO ESP32C3 底盘控制板",
        "spec": "USB-C，3.3 V GPIO，ESP32-C3；D2-D5 控 DRV8833，D6/D7 接 DualEye UART",
        "qty": "1",
        "required": "必买",
        "notes": "双板方案的底盘控制器。接收 DualEye UART 指令，执行电机限速、超时停车和 DRV8833 PWM/方向控制；ESP32-C3 SuperMini 可替代但必须重核引脚。",
        "est_cny": "25-45",
    },
    {
        "class": "运动",
    ...
    },
    {
        ...
        "qty": "2",
        "required": "必买",
        "notes": "四电机方案前后各一块 DRV8833：前板驱动左前/右前，后板驱动左后/右后；XIAO D2-D5 同时接到两块板输入。前后同侧电机作为同一逻辑侧，不要用一块 DRV8833 长期并联四个电机。",
        "est_cny": "10-20",
    },
    {
        "class": "控制/扩展",
        ...
        "qty": "1",
        "required": "推荐",
        "notes": "这是底盘板侧可选控制信号扩展板，不是电机驱动板。当前 XIAO D2-D5 可直接扇出到前/后两块 DRV8833，一般可省略；若底盘板 GPIO/PWM 不够再加。",
        "est_cny": "8-18",
    },
    {
        "class": "运动",
    ...
    },
    {
        ...
        "qty": "4",
        "required": "必买",
        "notes": "当前采用 64T 齿轮四轮方案。必须确认 3 mm D 孔可直接压入 N20 轴；每个齿轮直接装在对应 N20 轴上，不使用贯穿通轴；外圈建议套 O 型圈或热缩胶圈增加摩擦。",
        "est_cny": "16-40",
    },
    {
        "class": "运动/备选",
    ...
    },
    {
        ...
        "qty": "1",
        "required": "必买",
        "notes": "四个 N20 启动电流更高，避免虚标容量电池；若电机启动导致重启，优先更换高倍率电芯或降低 PWM。",
        "est_cny": "15-45",
    },
    {
        "class": "电源",
    ...
    },
    {
        ...
        "qty": "1",
        "required": "必买",
        "notes": "四个 N20 只从电机支路取电，不要让电机电流经过 ESP32/DualEye。若启动掉压，优先换更大电流模块并降低 PWM。",
        "est_cny": "8-20",
    },
    {
        "class": "电源",
...
BUY_QUERIES = {
    ...
    "Seeed Studio XIAO ESP32C3 底盘控制板": "Seeed XIAO ESP32C3 开发板",
    ...
    "18650 锂电池": "18650 锂电池 3.7V 高倍率 持续放电8A 带保护板",
    "单节 18650 电池盒带线": "单节 18650 电池盒 带线 1节",
    "5 V 升压模块": "锂电池升压模块 3.7V转5V 5A 大电流",
...
FIT_CHECKS = [
    ...
    ("电机控制引脚", "DualEye 外露备用引脚有限", "DRV8833 直连需 4 个 GPIO，且会挤占 HMI/音频/触摸资源", "风险已处理", "Mk.1 先不让 DualEye 直连 DRV8833；由 XIAO 底盘控制板提供 D2-D5 四路 PWM/DIR，必要时底盘板侧再加 PCA9685。"),
...
DECISION_MATRIX = [
    ...
    ("控制信号扩展", "PCA9685 I2C PWM", "推荐/可选", "底盘板 GPIO 不够时再用；当前 XIAO D2-D5 可直接控制两块 DRV8833"),
...
OPEN_ITEMS = [
    ...
    ("模块孔位", "两块 DRV8833、可选 PCA9685、升压模块、电池盒来自不同商家，最终钻孔前必须实测。"),
...
BRASS_CUTS = [
    ...
    ("2.0 mm", "中层前/后横梁", "2", "86", "172", "直线", "作为底盘控制板和两块 DRV8833 外侧保护框"),
...
ASSEMBLY_STEPS = [
    ...
    ("1", "底板开孔", "切 110 x 72 mm FR4/亚克力底板，钻 M2 铜柱孔、电池扎带孔和四个 N20 电机固定孔/长槽。", "底板边缘打磨，M2 螺丝能顺利穿过。"),
    ...
    ("4", "做中层保护框", "焊中层侧护栏和前后横梁，高度约 Z = 42-48 mm。该层保护底盘控制板、前/后两块 DRV8833 和线束。", "模块上方至少留 3 mm 空隙，USB-C 不被挡。"),
    ("5", "安装电子中层", "把 XIAO ESP32C3 底盘板、前 DRV8833、后 DRV8833、5 V 大电流升压固定在中层板或底板上，先插线再最终锁紧。", "驱动板焊盘不碰黄铜，信号线有余量。"),
    ...
    ("9", "分阶段通电", "先只给 DualEye 主控上电；再接 XIAO 底盘控制板；再接前/后两块 DRV8833 但悬空车轮；最后接 WS2812B 和喇叭。", "没有发热、异味、重启和异常电机抖动。"),
]
...
WIRING_ROWS = [
    ...
    ("电源共地", "电池 BAT- / 升压 OUT- / DualEye GND", "XIAO GND / 前 DRV8833 GND / 后 DRV8833 GND / WS2812B GND", "20-22 AWG 黑线或小分线板", "所有 GND 必须共地，否则 UART、电机和灯的控制信号会不稳定。"),
    ("升压到电机驱动", "5 V 升压 OUT+ / OUT-", "前 DRV8833 VM/GND；后 DRV8833 VM/GND", "20-22 AWG 红黑主线，短支路到两块驱动", "四个电机启动电流较高，VM 只接电机电源。不要从 ESP32/DualEye 3.3 V 给电机供电。"),
    ("DualEye 到 XIAO UART", "DualEye LCD1 Pin10 UART_TXD / Pin9 UART_RXD / GND", "XIAO D7(GPIO20) RX / D6(GPIO21) TX / GND", "26-28 AWG 信号线 + SH1.0 14P 转接线", "TX/RX 交叉连接，GND 必须共地。ESP32-S3 UART 是 3.3 V TTL，不要接 RS232；若底盘板 TX 是 5 V，先做分压或电平转换再进 DualEye RX。"),
    ("XIAO 逻辑供电", "5 V 升压 OUT+ / GND 或 USB-C", "XIAO 5V/VIN / GND", "24-26 AWG 红黑线", "仅在确认 XIAO 接口允许 5 V 时使用；调试期可先 USB-C 供电。电机 VM 不从 DualEye 板取电。"),
    ("XIAO 到前/后两块 DRV8833", "XIAO D2(GPIO4)/D3(GPIO5)/D4(GPIO6)/D5(GPIO7)", "两块 DRV8833 的 AIN1/AIN2/BIN1/BIN2 同名并联", "26-28 AWG 信号线，可在小分线点扇出", "D2-D5 同时给前/后两块 DRV8833；当前固件仍按左侧/右侧两路差速控制，不做四轮独立闭环。"),
    ("前 DRV8833 到前轮电机", "AOUT1/AOUT2；BOUT1/BOUT2", "左前 N20；右前 N20", "电机自带线或 24-26 AWG", "如果前进时某个轮子反转，交换该电机两根线即可。"),
    ("后 DRV8833 到后轮电机", "AOUT1/AOUT2；BOUT1/BOUT2", "左后 N20；右后 N20", "电机自带线或 24-26 AWG", "后轮方向要和同侧前轮一致；前后同侧绑定：左前+左后为左逻辑侧，右前+右后为右逻辑侧。"),
...
WIRE_LENGTHS = [
    ("20-22 AWG 红线", "约 1.0-1.2 m", "电池正极、电机支路、升压到前/后两块 DRV8833 主电源母线"),
    ("20-22 AWG 黑线", "约 1.0-1.2 m", "共地母线、电机支路负极、前后驱动板 GND"),
    ("26-28 AWG 多色信号线", "约 1.5-2.0 m", "UART、XIAO 到 DRV8833、WS2812B 数据和电平转换"),
...
ASSEMBLY_SIMULATION = [
    ...
    ("中层：主控与扩展", "中层板放 XIAO 底盘控制板、前/后两块 DRV8833、5 V 大电流升压和分线点；黄铜中层护栏高度 42-48 mm。", "能做出参考图中间裸露电路板和黄铜护栏的效果。", "底盘板/两块 DRV8833/升压模块孔位不统一，钻孔前必须按实物摆位；不要让模块焊盘碰黄铜。"),
...
def write_docs() -> None:
    ...
    write_text(
        ...
        - 紧固件标准：M2
        - 主控板安装：M2 铜柱，禁止使用 M3
        - 电机控制：DualEye 通过 UART 下发运动意图，XIAO ESP32C3 底盘板用 D2-D5 控制前/后两块 DRV8833
        ...
        电机链路是：DualEye 发 UART 安全运动意图 -> XIAO 底盘板输出左/右两侧 PWM/方向信号 -> 前后两块 DRV8833 输出电机电流 -> 四个 N20 电机转动。
    ...
    write_text(
        ...
        另一个实际修正是：Mk.1 先采用双板 UART 底盘控制，DualEye 不直接分配电机 GPIO；底盘板用 4 路逻辑输出同时驱动前后两块 DRV8833，实现四个 N20 的左右两侧差速。
    ...
    write_text(
        ...
        PCA9685 是可选的底盘板侧控制信号扩展板，不是必须的功率驱动板；当前 Mk.1 主路径不用 PCA9685。

        XIAO 底盘板连接两块 DRV8833：

        - XIAO D2 / GPIO4 -> 前、后 DRV8833 AIN1
        - XIAO D3 / GPIO5 -> 前、后 DRV8833 AIN2
        - XIAO D4 / GPIO6 -> 前、后 DRV8833 BIN1
        - XIAO D5 / GPIO7 -> 前、后 DRV8833 BIN2

        前 DRV8833 连接前轮电机：

        - AOUT1/AOUT2 -> 左前 N20 电机
        - BOUT1/BOUT2 -> 右前 N20 电机

        后 DRV8833 连接后轮电机：

        - AOUT1/AOUT2 -> 左后 N20 电机
        - BOUT1/BOUT2 -> 右后 N20 电机
    ...
    write_text(
        ...
        电气方案仍以 V1.0 定版为准：四只 N20 电机、四个 64T 齿轮轮、XIAO ESP32C3 底盘控制板、两块 DRV8833 电机驱动、WS2812B 外接车灯、AHCT/HCT 数据电平转换、小喇叭外放。PCA9685 只作为底盘板 GPIO 不够时的可选 PWM 扩展。
        ...
        4. 中层护栏高度建议 42-48 mm，用来保护底盘控制板、前/后两块 DRV8833 和线束。
        ...
        - XIAO D2-D5 到两块 DRV8833 的 4 根控制线尽量短，和电机电源线分开走；每根信号线可在小分线点扇出到前/后驱动板。
        ...
        4. 接 XIAO 底盘板，先通过 USB-C 串口发送 `AR1,PING` 或 `AR1,STOP`，确认底盘固件在线。
        5. 接前/后两块 DRV8833 控制线和电源线，但车轮悬空。用低 PWM 先测左侧前后轮，再测右侧前后轮。
        ...
        - 把 PCA9685 当电机驱动：它不是驱动板；若使用，也只是底盘板侧可选 PWM 扩展，DRV8833 才负责给电机输出电流。
    ...
    write_text(
        ...
        - 不建议做四轮独立闭环控制。Mk.1 只做四个物理电机的左右两侧差速，不加编码器/IMU 闭环。
...
def write_svg_overall() -> None:
    ...
    parts += [
        ...
        '<text x="93" y="99" fill="#333" stroke="none" text-anchor="middle">XIAO + 2xDRV8833</text>',
...
def build_pdf() -> None:
    ...
    class AtlasDrawing(Flowable):
        ...
        def _overall(self, c):
            ...
            self._text(c, 93, 64, "XIAO + 2xDRV8833", 3.1, "middle", "#333333")
        ...
        def _wiring(self, c):
            nodes = {
                ...
                "drv": (145, 48, 42, 22, "2xDRV8833"),
                "xiao": (75, 103, 42, 22, "XIAO 底盘板"),
                "mot": (145, 17, 42, 18, "4xN20 电机"),
                "spk": (150, 80, 42, 15, "喇叭"),
            ...
            wire("xiao", "drv", "#1f4e79")
            wire("board", "xiao", "#1f4e79")
            wire("drv", "mot", "#333333")
            ...
            self._text(c, 123, 83, "D2-D5 扇出", 2.8, "middle", "#1f4e79")
            self._text(c, 126, 121, "UART", 2.8, "middle", "#1f4e79")
    ...
    story += [
        ...
        table(
            [
                ...
                ["电机驱动", "必备两块 DRV8833 双路 H 桥电机驱动板"],
                ["电机控制", "DualEye UART -> XIAO D2-D5 -> 前后两块 DRV8833"],
