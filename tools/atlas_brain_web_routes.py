...
def render_acceptance_page(bridge: Any, *, brain_port: int) -> str:
    skip_cmd = (
        f"python3 tools/check_atlas_experience.py --brain-url http://127.0.0.1:{brain_port} "
        "--skip-dualeye --output-json /tmp/atlas_experience.json --output-md /tmp/atlas_experience.md"
    )
    device_cmd = (
        f"python3 tools/check_atlas_experience.py --brain-url http://127.0.0.1:{brain_port} "
        f"--dualeye-url {bridge.dualeye_url} --output-json /tmp/atlas_experience.json --output-md /tmp/atlas_experience.md"
    )
...
<section><h2>实机验收命令</h2><p class="muted">报告会写入 JSON 和 Markdown，可直接发给我们复盘；页面不会显示密钥值。</p><button onclick="copyCmd('skipCmd')">复制无真机命令</button><button onclick="copyCmd('deviceCmd')">复制真机命令</button><code id="skipCmd" class="cmd">{html.escape(skip_cmd)}</code><code id="deviceCmd" class="cmd">{html.escape(device_cmd)}</code></section>
...
async function copyCmd(id){{const el=document.getElementById(id);try{{await navigator.clipboard.writeText(el.textContent);el.style.borderColor='#74e0a3'}}catch(e){{document.getElementById('out').textContent=el.textContent}}}}
...
function providerMissingText(pd){{const out=[];for(const k of ['llm','asr','tts']){{const x=pd[k]||{{}};out.push(k.toUpperCase()+': '+(x.configured?'ready':('缺 '+((x.missing_env||[]).join(', ')||'未知配置'))))}}return out}}
async function loadLiveStatus(){{const box=document.getElementById('liveStatus');try{{const [live,diag,stream,providers,report]=await Promise.all([getJson('/api/device/live'),getJson('/api/runtime/diagnostics'),getJson('/api/audio/stream/status'),getJson('/api/providers'),getJson('/api/acceptance/report')]);const d=diag.data?.diagnostics||{{}};const scene=live.data?.scene||{{}};const latestTool=d.latest_tool||{{}};const latestTurn=d.latest_turn||{{}};const failure=d.latest_failure||{{}};const s=stream.data?.stream||stream.data?.last_stream||{{}};const provSummary=providers.data?.summary||{{}};const providerDiag=providers.data?.provider_diagnostics||{{}};const exp=report.data?.experience_status?.summary||{{}};box.innerHTML=[liveCard('DualEye '+(live.data?.online?'在线':'离线'),live.data?.online?'pass':'warn',['scene='+(scene.state||''),'title='+(scene.title||''),'hint='+(scene.hint||scene.subtitle||'')]),liveCard('最近 Turn',latestTurn.ok?'pass':(latestTurn.failed_stage?'fail':'warn'),['source='+(latestTurn.source||''),'failed_stage='+(latestTurn.failed_stage||'ready'),'reply='+(latestTurn.reply||'')]),liveCard('最近工具',latestTool.ok?'pass':(latestTool.name?'fail':'warn'),['name='+(latestTool.name||''),'risk='+(latestTool.risk||''),'error='+(latestTool.error||latestTool.error_code||'')]),liveCard('诊断',d.ok?'pass':'fail',['stage='+(d.stage||'ready'),'next='+(d.next_step||''),'failure='+(failure.detail||failure.stage||'')]),liveCard('Provider 缺项',providerDiag.llm?.configured&&providerDiag.asr?.configured&&providerDiag.tts?.configured?'pass':'warn',providerMissingText(providerDiag).concat(['只显示缺项，不显示密钥值'])),liveCard('Provider 摘要',Number(provSummary.configured_providers||0)>0?'warn':'warn',['configured='+(provSummary.configured_providers||0)+'/'+(provSummary.provider_count||0),'LLM/ASR/TTS 详见 /api/providers']),liveCard('OPUS Stream',s.frames?'pass':'warn',['frames='+(s.frames||s.atlas_frames||0),'bytes='+(s.bytes||0),'gaps='+(s.sequence_gaps||0)]),liveCard('体验线评分','warn',['voice='+(exp.voice_continuous_conversation??''),'dualeye='+(exp.dualeye_pet_apps??''),'tools='+(exp.brain_tools_diagnostics??'')])].join('')}}catch(e){{box.innerHTML=liveCard('实机状态读取失败','fail',[e.message||String(e)])}}}}
