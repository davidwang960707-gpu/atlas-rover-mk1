...
from atlas_brain_acceptance import build_acceptance_report, build_runtime_score_payload
...
class Bridge:
    ...
    def runtime_score_payload(self) -> dict[str, Any]:
        return build_runtime_score_payload(self)
    ...
    def query_weather(self, location: str = "") -> dict[str, Any]:
        return query_weather_provider(location)
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:
            ...
            if path == "/ota/manifest":
                ...
                self.send_json(build_ota_manifest_response(package_base_url))
                return
            if path == "/api/ota/packages":
                package_base_url = f"http://{local_lan_ip()}:{self.server.server_address[1]}/ota/package"
                self.send_json(build_ota_packages_response(package_base_url))
                return
            ...
            if path == "/devices":
                ...
                return
            if path == "/acceptance":
                handle_acceptance_page(self, bridge, brain_port=int(self.server.server_address[1]))
                return
            if path == "/app" or (path.startswith("/devices/") and path.endswith("/app")):
                ...
                return
            if path == "/tools":
                handle_tools_page(self, bridge, rover_skills_enabled=ENABLE_ROVER_SKILLS)
                return
