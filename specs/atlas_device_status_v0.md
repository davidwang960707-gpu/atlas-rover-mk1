...
`experience.ui.current_page` 是应用状态里请求的页面，`experience.ui.rendered_page` 是 scene resolver 最终建议渲染的页面。两者不一致时，通常说明运行时状态、Brain 离线兜底或语音场景覆盖了手动页面。`left_screen/right_screen` 是固件对双圆屏内容的预期，不代表硬件像素采样；用于真机看到单屏亮、宠物头未出现或页面切换无效时快速排障。
