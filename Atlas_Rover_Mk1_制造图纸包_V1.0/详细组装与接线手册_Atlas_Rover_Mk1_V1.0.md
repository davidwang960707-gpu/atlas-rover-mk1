...
电气方案仍以 V1.0 定版为准：四只 N20 电机、四个 64T 齿轮轮、底盘控制板、两块 DRV8833 电机驱动、WS2812B 外接车灯、AHCT/HCT 数据电平转换、小喇叭外放。PCA9685 只作为底盘板 GPIO 不够时的可选 PWM 扩展。
...
| 1 | 底板开孔 | 切 110 x 72 mm FR4/亚克力底板；若四个 N20 过挤，可放宽到 110 x 78-80 mm。钻 M2 铜柱孔、电池扎带孔和四个电机长槽，前后轮轴线分别距前/后边约 20 mm。 | 底板边缘打磨，M2 螺丝能顺利穿过，四个电机夹能微调。 |
...
| 5 | 安装电子中层 | 把底盘控制板、前 DRV8833、后 DRV8833 固定在中层板或底板上，先插线再最终锁紧。 | 驱动板焊盘不碰黄铜，信号线有余量，每块驱动板 VM/GND 旁有电容。 |
...
4. 中层护栏高度建议 42-48 mm，用来保护底盘控制板、电机驱动板和线束。
...
| XIAO 到两块 DRV8833 | XIAO D2/D3/D4/D5 | 前/后 DRV8833 的 AIN1 / AIN2 / BIN1 / BIN2 | 26-28 AWG 信号线 | D2=GPIO4 左侧正转，D3=GPIO5 左侧反转，D4=GPIO6 右侧正转，D5=GPIO7 右侧反转；同一信号扇出到两块驱动板。 |
...
- 电机线按四角成对绞合，从左右两边贴底框走，左前/右前进前 DRV8833，左后/右后进后 DRV8833。
...
5. 接两块 DRV8833 控制线和电源线，但车轮悬空。用低 PWM 先测左前/左后，再测右前/右后。
6. 若方向反了，只交换对应电机的 AOUT 或 BOUT 两根线；确认左前/左后同向、右前/右后同向。
...
- 把四个电机长期并到一块 DRV8833：不要这样做，四电机方案使用两块 DRV8833 分担电流和散热。
