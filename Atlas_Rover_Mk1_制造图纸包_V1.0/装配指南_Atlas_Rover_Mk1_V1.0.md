...
3. 中层电子板：底盘控制板、两块 DRV8833、走线分配、推荐外接车灯/RGB。
...
黄铜轮拱和侧护栏可以做出四轮小车的视觉姿态，四个 64T 齿轮轮都参与支撑和驱动。

## 前后桥和 64T 齿轮轮

当前采用 64T 钢齿轮作为四个驱动轮轮芯，需要满足这几个条件：

- 优先买外径 30-36 mm、中心 3 mm D 孔、厚 3-6 mm 的 64T 齿轮，数量 4 个。
- 如果齿轮不是 3 mm D 孔，必须加 3 mm D 孔轮毂或 3D 打印夹片，不建议只用胶水粘在 N20 轴上。
- 钢齿轮外圈要套 O 型圈、热缩圈或橡胶圈，否则容易打滑、噪声大，也会刮桌面。
- 前后桥都是左右两个 N20 独立半桥，不使用贯穿通轴；左右侧必须能按差速独立转动。
- 前后同侧电机接同一组控制信号：左前/左后同向，右前/右后同向。
- 迷你万向轮不再作为主支撑，只保留为两轮回退或台架调试备件。

前桥轴线建议距底板前边 18-22 mm，后桥轴线建议距底板后边 18-22 mm。四个 N20 输出轴朝外，电机本体放在底板四角内侧，中间留给电池盒、底盘板、两块 DRV8833 和线束。更完整的结构说明见 `docs/后桥前桥与64T齿轮轮方案_Atlas_Rover_Mk1.md`。
...
| 1 | 底板开孔 | 切 110 x 72 mm FR4/亚克力底板；若四个电机挤，可放宽到 110 x 78-80 mm。钻 M2 铜柱孔、电池扎带孔、四个电机长槽；前后轮轴线分别距前/后边约 20 mm。 | 底板边缘打磨，M2 螺丝能顺利穿过，前后同侧 N20 输出轴基本平行，四轮中心高度一致。 |
...
| 4 | 做中层保护框 | 焊中层侧护栏和前后横梁，高度约 Z = 42-48 mm。该层保护底盘控制板、两块 DRV8833 和线束。 | 模块上方至少留 3 mm 空隙，USB-C 不被挡。 |
| 5 | 安装电子中层 | 把底盘控制板、前 DRV8833、后 DRV8833 固定在中层板或底板上，先插线再最终锁紧。 | 驱动板焊盘不碰黄铜，信号线有余量，两块驱动板 VM/GND 附近都有电容。 |
