...
4. 在管理页保存 Wi-Fi、LLM/API 和安全设置。
5. 保存 Wi-Fi 后建议重启，设备会优先连接路由器；如果连接失败，会回落到 SoftAP/APSTA。
...
| 模式 | 允许的运动来源 | 说明 |
|---|---|---|
| `manual` | Web 手动控制 | 用户在管理页点前进/后退/转向；DualEye 裁剪速度和时长后通过 UART 下发 |
