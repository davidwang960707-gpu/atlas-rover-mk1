...
#define ATLAS_AUDIO_SERVICE_QUEUE_LEN 1u
#define ATLAS_AUDIO_SERVICE_TASK_STACK 32768u
#define ATLAS_AUDIO_SERVICE_TASK_PRIORITY 3u

static const char *TAG = "atlas_audio_service";

typedef struct {
    atlas_audio_service_job_fn_t fn;
    void *ctx;
    SemaphoreHandle_t done;
    esp_err_t result;
} atlas_audio_service_job_t;

static atlas_audio_service_status_t s_status;
static atlas_audio_service_now_ms_fn_t s_now_ms;
...
{
    return s_now_ms == NULL ? 0u : s_now_ms();
}
...
static void note_mode_locked(atlas_audio_service_mode_t mode, const char *action)
{
    s_status.mode = mode;
...
static void note_mode(atlas_audio_service_mode_t mode, const char *action)
{
    if (status_lock()) {
...
static void finish_job_mode_locked(esp_err_t err)
{
    if (err == ESP_OK) {
        ...
        note_mode_locked(s_status.continuous_enabled ? ATLAS_AUDIO_SERVICE_MODE_MONITORING : ATLAS_AUDIO_SERVICE_MODE_IDLE,
                         s_status.continuous_enabled ? "turn_done_monitoring" : "turn_done_idle");
    } else {
        s_status.consecutive_failures++;
        note_mode_locked(s_status.continuous_enabled ? ATLAS_AUDIO_SERVICE_MODE_MONITORING : ATLAS_AUDIO_SERVICE_MODE_ERROR,
...
static void audio_service_worker(void *arg)
{
    ...
    for (;;) {
        atlas_audio_service_job_t *job = NULL;
...
void atlas_audio_service_init(atlas_audio_service_now_ms_fn_t now_ms_fn)
{
    ...
    if (status_lock()) {
        ...
        s_status.mode = ATLAS_AUDIO_SERVICE_MODE_IDLE;
        s_status.last_error = ESP_OK;
    ...
    if (s_job_queue == NULL) {
        s_job_queue = xQueueCreate(ATLAS_AUDIO_SERVICE_QUEUE_LEN, sizeof(atlas_audio_service_job_t *));
    }
    if (s_job_queue != NULL && s_worker_task == NULL) {
        BaseType_t ok = xTaskCreateWithCaps(audio_service_worker,
                                            "atlas_audio_svc",
                                            ATLAS_AUDIO_SERVICE_TASK_STACK,
                                            NULL,
                                            ATLAS_AUDIO_SERVICE_TASK_PRIORITY,
                                            &s_worker_task,
...
{
    switch (mode) {
    case ATLAS_AUDIO_SERVICE_MODE_IDLE:
        return "idle";
    case ATLAS_AUDIO_SERVICE_MODE_MONITORING:
        return "monitoring";
    case ATLAS_AUDIO_SERVICE_MODE_RECORDING:
        return "recording";
    case ATLAS_AUDIO_SERVICE_MODE_TRANSCRIBING:
        return "transcribing";
    case ATLAS_AUDIO_SERVICE_MODE_THINKING:
        return "thinking";
    case ATLAS_AUDIO_SERVICE_MODE_PLAYING:
        return "playing";
    case ATLAS_AUDIO_SERVICE_MODE_COOLDOWN:
        return "cooldown";
    case ATLAS_AUDIO_SERVICE_MODE_ERROR:
        return "error";
    default:
        return "unknown";
    }
...
void atlas_audio_service_set_continuous_enabled(bool enabled)
{
    if (status_lock()) {
        ...
        note_mode_locked(enabled ? ATLAS_AUDIO_SERVICE_MODE_MONITORING : ATLAS_AUDIO_SERVICE_MODE_IDLE,
...
void atlas_audio_service_note_failure(const char *reason, esp_err_t err)
{
    if (status_lock()) {
        ...
        note_mode_locked(ATLAS_AUDIO_SERVICE_MODE_ERROR, reason == NULL ? "error" : reason);
        status_unlock();
    }
}

bool atlas_audio_service_is_muted(uint32_t *remaining)
{
    uint32_t left = 0;
    bool muted = false;
    if (status_lock()) {
...
esp_err_t atlas_audio_service_run_turn(atlas_audio_service_job_fn_t fn, void *ctx, uint32_t timeout_ms)
{
    ...
    if (timeout_ms != 0u) {
        atlas_audio_service_note_failure("finite timeout not supported", ESP_ERR_INVALID_ARG);
        return ESP_ERR_INVALID_ARG;
    }
    if (s_job_queue == NULL || s_worker_task == NULL) {
        atlas_audio_service_note_failure("worker not started", ESP_ERR_INVALID_STATE);
    ...
    if (busy) {
        atlas_audio_service_note_failure("audio service busy", ESP_ERR_INVALID_STATE);
        return ESP_ERR_INVALID_STATE;
    }

    atlas_audio_service_job_t job = {
        .fn = fn,
        .ctx = ctx,
    ...
    if (job.done == NULL) {
        atlas_audio_service_note_failure("job semaphore failed", ESP_ERR_NO_MEM);
        return ESP_ERR_NO_MEM;
    }

    atlas_audio_service_job_t *job_ptr = &job;
    if (xQueueSend(s_job_queue, &job_ptr, 0) != pdTRUE) {
        vSemaphoreDelete(job.done);
        atlas_audio_service_note_failure("turn queue full", ESP_ERR_INVALID_STATE);
        return ESP_ERR_INVALID_STATE;
    }

    if (xSemaphoreTake(job.done, portMAX_DELAY) != pdTRUE) {
        vSemaphoreDelete(job.done);
        atlas_audio_service_note_failure("turn timeout", ESP_ERR_TIMEOUT);
...
esp_err_t atlas_audio_service_submit_turn(atlas_audio_service_job_fn_t fn, void *ctx)
{
    ...
    atlas_audio_service_job_t *job = (atlas_audio_service_job_t *)calloc(1, sizeof(*job));
    if (job == NULL) {
        atlas_audio_service_note_failure("async job alloc failed", ESP_ERR_NO_MEM);
        return ESP_ERR_NO_MEM;
    }
    job->fn = fn;
    job->ctx = ctx;
    ...
    if (xQueueSend(s_job_queue, &job, 0) != pdTRUE) {
        free(job);
        atlas_audio_service_note_failure("async queue full", ESP_ERR_INVALID_STATE);
        return ESP_ERR_INVALID_STATE;
    }
    return ESP_OK;
}
...
esp_err_t atlas_audio_service_measure_mic(uint16_t duration_ms, atlas_audio_mic_level_t *level)
{
    ...
    note_mode(ATLAS_AUDIO_SERVICE_MODE_MONITORING, "measure_mic");
    const esp_err_t err = atlas_audio_measure_mic(duration_ms, level);
    if (status_lock()) {
        s_status.last_error = err;
        status_unlock();
    }
    if (err != ESP_OK) {
        note_mode(ATLAS_AUDIO_SERVICE_MODE_ERROR, "measure_error");
    } else {
        ...
        if (!continuous_enabled) {
            note_mode(ATLAS_AUDIO_SERVICE_MODE_IDLE, "measure_done");
        }
    }
    return err;
}
                                          ...
                                          atlas_audio_mic_level_t *level)
{
    ...
    note_mode(ATLAS_AUDIO_SERVICE_MODE_RECORDING, "capture_wav");
    const esp_err_t err = atlas_audio_capture_wav(duration_ms, wav_data, wav_size, level);
    ...
    if (err == ESP_OK) {
        note_mode(job_running ? ATLAS_AUDIO_SERVICE_MODE_THINKING : ATLAS_AUDIO_SERVICE_MODE_IDLE, "capture_done");
    } else {
        atlas_audio_service_note_failure("capture_error", err);
    }
    return err;
}
...
esp_err_t atlas_audio_service_play_wav_pcm(const uint8_t *wav_data, size_t wav_size, uint8_t volume)
{
    ...
    note_mode(ATLAS_AUDIO_SERVICE_MODE_PLAYING, "play_wav");
    const esp_err_t err = atlas_audio_play_wav_pcm(wav_data, wav_size, volume);
    bool job_running = false;
    if (status_lock()) {
        s_status.last_error = err;
        job_running = s_job_running;
    ...
    set_operation_busy(false);
    if (err == ESP_OK) {
        note_mode(job_running ? ATLAS_AUDIO_SERVICE_MODE_COOLDOWN : ATLAS_AUDIO_SERVICE_MODE_IDLE, "play_done");
    } else {
        atlas_audio_service_note_failure("play_error", err);
    }
    return err;
}
...
void atlas_audio_service_get_status(atlas_audio_service_status_t *status)
{
    ...
    (void)atlas_audio_service_is_muted(NULL);
    if (status_lock()) {
        *status = s_status;
        status_unlock();
    } else {
...
size_t atlas_audio_service_write_json(char *dst, size_t dst_size)
{
    ...
    atlas_audio_service_status_t status;
    atlas_audio_service_get_status(&status);
    ...
    return snprintf(dst,
                    ...
                    atlas_audio_service_mode_name(status.mode),
