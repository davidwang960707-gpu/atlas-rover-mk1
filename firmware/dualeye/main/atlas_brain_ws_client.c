...
static const char *TAG = "atlas_brain_ws";

#define ATLAS_BRAIN_WS_TASK_STACK 8192u
#define ATLAS_BRAIN_WS_TASK_PRIORITY 4u
#define ATLAS_BRAIN_WS_CONNECT_WAIT_MS 9000u
#define ATLAS_BRAIN_WS_RECONNECT_MS 3000u
#define ATLAS_BRAIN_WS_HEALTH_RETRY_MS 3500u
#define ATLAS_BRAIN_WS_PING_MS 10000u
#define ATLAS_BRAIN_WS_SEND_TIMEOUT_MS 1200u
#define ATLAS_BRAIN_WS_TURN_RESPONSE_MAX 4096u
#define ATLAS_BRAIN_WS_TTS_MAX (512u * 1024u)
#define ATLAS_BRAIN_WS_TURN_TIMEOUT_MS 90000u
...
static atlas_brain_ws_now_ms_fn_t s_now_ms;
static atlas_config_t s_config_snapshot;
static atlas_brain_ws_status_t s_status;
static esp_websocket_client_handle_t s_active_client;

static uint32_t s_turn_request_id;
static bool s_turn_waiting;
static char s_turn_response[ATLAS_BRAIN_WS_TURN_RESPONSE_MAX];
static size_t s_turn_response_len;
static char s_turn_text_rx[ATLAS_BRAIN_WS_TURN_RESPONSE_MAX];
...
static uint32_t brain_ws_now_ms(void)
{
    if (s_now_ms != NULL) {
        return s_now_ms();
    }
...
static esp_err_t brain_ws_ensure_mutex(void)
{
    if (s_mutex == NULL) {
...
static void brain_ws_lock(void)
{
    if (brain_ws_ensure_mutex() == ESP_OK) {
        (void)xSemaphoreTake(s_mutex, portMAX_DELAY);
    }
}

static void brain_ws_unlock(void)
{
    if (s_mutex != NULL) {
...
static void brain_ws_set_stage(const char *stage, esp_err_t err)
{
    brain_ws_lock();
    strlcpy(s_status.stage, stage == NULL ? "unknown" : stage, sizeof(s_status.stage));
    s_status.last_error = err;
    if (err != ESP_OK) {
        s_status.last_error_ms = brain_ws_now_ms();
    }
    brain_ws_unlock();
}

static bool brain_ws_build_url(const char *base_url, char *out, size_t out_size)
{
    if (base_url == NULL || base_url[0] == '\0' || out == NULL || out_size == 0) {
        return false;
    }

    char tmp[ATLAS_LLM_BASE_URL_MAX + 16] = "";
...
static bool brain_ws_build_http_url(const char *base_url, const char *path, char *out, size_t out_size)
{
    if (base_url == NULL || base_url[0] == '\0' || path == NULL || out == NULL || out_size == 0) {
        return false;
    }

    char tmp[ATLAS_LLM_BASE_URL_MAX + 1] = "";
    strlcpy(tmp, base_url, sizeof(tmp));
    while (strlen(tmp) > 0 && tmp[strlen(tmp) - 1] == '/') {
        tmp[strlen(tmp) - 1] = '\0';
    }

    char base[ATLAS_LLM_BASE_URL_MAX + 16] = "";
    if (strncmp(tmp, "ws://", 5) == 0) {
        snprintf(base, sizeof(base), "http://%s", tmp + 5);
    } else if (strncmp(tmp, "wss://", 6) == 0) {
        snprintf(base, sizeof(base), "https://%s", tmp + 6);
    } else if (strncmp(tmp, "http://", 7) == 0 || strncmp(tmp, "https://", 8) == 0) {
        strlcpy(base, tmp, sizeof(base));
    } else {
        snprintf(base, sizeof(base), "http://%s", tmp);
    }
...
static bool brain_ws_config_enabled(const atlas_config_t *config, char *url, size_t url_size)
{
    if (config == NULL || strcmp(config->llm.mode, "host") != 0) {
        return false;
    }
    return brain_ws_build_url(config->llm.base_url, url, url_size);
}

static esp_err_t brain_ws_probe_health(const atlas_config_t *config, int *status_code)
{
    if (status_code != NULL) {
        *status_code = 0;
    }
    if (config == NULL || config->llm.base_url[0] == '\0') {
        return ESP_ERR_INVALID_ARG;
    }

    char health_url[ATLAS_LLM_BASE_URL_MAX + 16] = "";
    if (!brain_ws_build_http_url(config->llm.base_url, "/health", health_url, sizeof(health_url))) {
        return ESP_ERR_INVALID_ARG;
    }
...
static esp_err_t brain_ws_send_json(esp_websocket_client_handle_t client, const char *json)
{
    if (client == NULL || json == NULL || !esp_websocket_client_is_connected(client)) {
        return ESP_ERR_INVALID_STATE;
    }
    const int sent = esp_websocket_client_send_text(client, json, strlen(json), pdMS_TO_TICKS(ATLAS_BRAIN_WS_SEND_TIMEOUT_MS));
    if (sent < 0) {
        return ESP_FAIL;
    }
    brain_ws_lock();
...
static esp_err_t brain_ws_send_event(esp_websocket_client_handle_t client, const char *event, const char *detail)
{
    const atlas_runtime_state_t runtime_state = atlas_runtime_get_state();
    char payload[512];
    snprintf(payload,
             sizeof(payload),
             "{\"type\":\"%s\",\"event\":\"%s\",\"protocol\":\"atlas.brain.session.v1\","
             "\"device_id\":\"dualeye\",\"runtime\":\"%s\",\"detail\":\"%s\","
             "\"free_heap\":%u,\"ts_ms\":%" PRIu32 "}",
             event,
             event,
             atlas_runtime_state_name(runtime_state),
             detail == NULL ? "" : detail,
             (unsigned)heap_caps_get_free_size(MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT),
             brain_ws_now_ms());
    return brain_ws_send_json(client, payload);
}

static esp_err_t brain_ws_ensure_request_sync(void)
{
...
static void brain_ws_reset_turn_buffers_locked(void)
{
    s_turn_response[0] = '\0';
...
static void brain_ws_handle_text_payload(const char *json)
{
    if (json == NULL) {
        return;
    }
    ...
    if (waiting && strstr(json, "turn.tts.begin") != NULL) {
        ...
        if (bytes > 0 && bytes <= ATLAS_BRAIN_WS_TTS_MAX) {
            free(s_turn_tts_data);
...
static void brain_ws_handle_binary_payload(const uint8_t *data, size_t len, size_t offset, size_t payload_len)
{
    if (data == NULL || len == 0) {
        return;
    }
...
static void brain_ws_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    (void)handler_args;
    (void)base;
    esp_websocket_event_data_t *data = (esp_websocket_event_data_t *)event_data;
    if (event_id == WEBSOCKET_EVENT_ERROR) {
        brain_ws_set_stage("socket_error", ESP_FAIL);
        return;
    }
    ...
    if (data->op_code == 0x1) {
        ...
        char text[ATLAS_BRAIN_WS_TURN_RESPONSE_MAX];
        text[0] = '\0';
        ...
        brain_ws_unlock();
        if (text[0] != '\0') {
            brain_ws_handle_text_payload(text);
        }
    } else if (data->op_code == 0x2) {
        brain_ws_handle_binary_payload((const uint8_t *)data->data_ptr, data_len, offset, payload_len);
    }
}

static void brain_ws_task(void *arg)
{
    (void)arg;
    char url[sizeof(s_status.url)] = "";
    if (!brain_ws_config_enabled(&s_config_snapshot, url, sizeof(url))) {
        brain_ws_set_stage("disabled", ESP_OK);
        brain_ws_lock();
    ...
    while (!s_stop_requested) {
        ...
        if (!wifi.sta_connected) {
            brain_ws_set_stage("waiting_wifi", ESP_OK);
            vTaskDelay(pdMS_TO_TICKS(1000));
            continue;
        }

        int health_status = 0;
        const esp_err_t health_err = brain_ws_probe_health(&s_config_snapshot, &health_status);
        if (health_err != ESP_OK) {
            ...
            ESP_LOGW(TAG,
                     ...
                     health_status,
                     s_status.url);
            vTaskDelay(pdMS_TO_TICKS(ATLAS_BRAIN_WS_HEALTH_RETRY_MS));
            continue;
        }

        brain_ws_set_stage("connecting", ESP_OK);
        esp_websocket_client_config_t ws_cfg = {
            ...
            .task_name = "atlas_ws_brain",
            .task_stack = 8192,
            .task_prio = 4,
        };
        esp_websocket_client_handle_t client = esp_websocket_client_init(&ws_cfg);
        if (client == NULL) {
            brain_ws_set_stage("init_failed", ESP_FAIL);
            vTaskDelay(pdMS_TO_TICKS(ATLAS_BRAIN_WS_RECONNECT_MS));
            continue;
        }
        esp_err_t err = esp_websocket_register_events(client,
                                                      WEBSOCKET_EVENT_ANY,
                                                      brain_ws_event_handler,
                                                      NULL);
        if (err != ESP_OK) {
            brain_ws_set_stage("event_failed", err);
        ...
        if (err != ESP_OK) {
            brain_ws_set_stage("start_failed", err);
            esp_websocket_client_destroy(client);
            vTaskDelay(pdMS_TO_TICKS(ATLAS_BRAIN_WS_RECONNECT_MS));
            continue;
        }

        const uint32_t started = brain_ws_now_ms();
        while (!s_stop_requested &&
               !esp_websocket_client_is_connected(client) &&
               brain_ws_now_ms() - started < ATLAS_BRAIN_WS_CONNECT_WAIT_MS) {
            vTaskDelay(pdMS_TO_TICKS(80));
        }

        if (!esp_websocket_client_is_connected(client)) {
            brain_ws_set_stage("connect_timeout", ESP_ERR_TIMEOUT);
        ...
        s_status.connects++;
        s_status.last_connected_ms = brain_ws_now_ms();
        s_active_client = client;
        brain_ws_unlock();
        brain_ws_set_stage("connected", ESP_OK);
        (void)brain_ws_send_event(client, "hello", "dualeye boot/session");

        uint32_t last_ping = brain_ws_now_ms();
        while (!s_stop_requested && esp_websocket_client_is_connected(client)) {
            const uint32_t now = brain_ws_now_ms();
            if (now - last_ping >= ATLAS_BRAIN_WS_PING_MS) {
                (void)brain_ws_send_event(client, "ping", "periodic_state");
                last_ping = now;
            }
        ...
        brain_ws_unlock();
        brain_ws_set_stage(s_stop_requested ? "stopped" : "disconnected", ESP_OK);
        esp_websocket_client_stop(client);
        esp_websocket_client_destroy(client);
...
{
    ESP_RETURN_ON_ERROR(brain_ws_ensure_mutex(), TAG, "mutex");
    ESP_RETURN_ON_ERROR(brain_ws_ensure_request_sync(), TAG, "request sync");
    if (config == NULL) {
        return ESP_ERR_INVALID_ARG;
    }
    if (s_task != NULL) {
    ...
    char url[sizeof(s_status.url)] = "";
    const bool enabled = brain_ws_config_enabled(config, url, sizeof(url));
    ...
    brain_ws_reset_turn_buffers_locked();
    ...
    if (!enabled) {
        ESP_LOGI(TAG, "Atlas Brain WS disabled; llm.mode must be host and base_url must be set");
        return ESP_OK;
    }

    const BaseType_t ok = xTaskCreate(brain_ws_task,
                                      "atlas_brain_ws",
                                      ATLAS_BRAIN_WS_TASK_STACK,
                                      NULL,
                                      ATLAS_BRAIN_WS_TASK_PRIORITY,
                                      &s_task);
    if (ok != pdPASS) {
        brain_ws_set_stage("task_failed", ESP_ERR_NO_MEM);
        return ESP_ERR_NO_MEM;
    }
    ESP_LOGI(TAG, "Atlas Brain WS client starting: %s", url);
    return ESP_OK;
}
                                         ...
                                         uint32_t timeout_ms)
{
    ...
    *response_len = 0;
    ...
    snprintf(begin,
             sizeof(begin),
             "{\"type\":\"turn.audio.begin\",\"protocol\":\"atlas.brain.session.v1\","
             "\"request_id\":%" PRIu32 ",\"device_id\":\"dualeye\","
    ...
    esp_err_t err = brain_ws_send_json(client, begin);
    if (err == ESP_OK) {
        const int sent = esp_websocket_client_send_bin(client,
                                                       (const char *)wav,
                                                       (int)wav_len,
                                                       pdMS_TO_TICKS(ATLAS_BRAIN_WS_SEND_TIMEOUT_MS * 4u));
    ...
    const uint32_t wait_ms = timeout_ms == 0 ? ATLAS_BRAIN_WS_TURN_TIMEOUT_MS : timeout_ms;
...
size_t atlas_brain_ws_client_write_json(char *dst, size_t dst_size)
{
    ...
    atlas_brain_ws_status_t status;
    atlas_brain_ws_client_get_status(&status);
