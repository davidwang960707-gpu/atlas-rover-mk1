...
#define ATLAS_COMMON_OPUS_STREAM_TASK_STACK 24576u
#define ATLAS_COMMON_OPUS_STREAM_TASK_PRIORITY 4u
#define ATLAS_COMMON_OPUS_STREAM_CONNECT_TIMEOUT_MS 8000u
#define ATLAS_COMMON_OPUS_STREAM_SEND_TIMEOUT_MS 1500u
#define ATLAS_COMMON_OPUS_STREAM_MAX_DURATION_MS (5u * 60u * 1000u)

static TaskHandle_t s_stream_task;
static SemaphoreHandle_t s_stream_mutex;
static volatile bool s_stream_stop_requested;
static atlas_common_opus_stream_backend_t s_backend;
static char s_device_id[32] = "device";
static char s_stream_task_name[32] = "atlas_common_opus_ws";
static char s_websocket_task_name[32] = "atlas_ws_audio";
static atlas_common_opus_stream_status_t s_stream_status = {
    .frame_ms = ATLAS_COMMON_OPUS_FRAME_MS,
    .sample_rate = ATLAS_COMMON_OPUS_SAMPLE_RATE,
    .last_error = ESP_OK,
    .stage = "idle",
};
...
void atlas_common_opus_stream_init(const atlas_common_opus_stream_backend_t *backend)
{
    ...
    strlcpy(s_device_id, s_backend.device_id == NULL ? "device" : s_backend.device_id, sizeof(s_device_id));
    strlcpy(s_stream_task_name,
            s_backend.stream_task_name == NULL ? "atlas_common_opus_ws" : s_backend.stream_task_name,
            sizeof(s_stream_task_name));
    strlcpy(s_websocket_task_name,
            s_backend.websocket_task_name == NULL ? "atlas_ws_audio" : s_backend.websocket_task_name,
            sizeof(s_websocket_task_name));
    s_backend.device_id = s_device_id;
    s_backend.stream_task_name = s_stream_task_name;
    s_backend.websocket_task_name = s_websocket_task_name;
}

static esp_err_t common_capture_pcm(int16_t *samples, size_t sample_count, atlas_common_opus_mic_level_t *level)
{
    if (s_backend.capture_pcm_fn == NULL) {
        return ESP_ERR_INVALID_STATE;
    }
    return s_backend.capture_pcm_fn(samples, sample_count, level, s_backend.ctx);
}

static bool common_is_muted(uint32_t *remaining_ms)
{
    if (s_backend.is_muted_fn == NULL) {
        if (remaining_ms != NULL) {
            *remaining_ms = 0;
        }
        return false;
    }
    return s_backend.is_muted_fn(remaining_ms, s_backend.ctx);
}

static void common_note_stage(bool ok, const char *action)
{
    if (s_backend.note_stage_fn != NULL) {
        s_backend.note_stage_fn(ok, action, s_backend.ctx);
    }
}
...
static void write_stream_binary_header(uint8_t *dst,
                                       ...
                                       const atlas_common_opus_mic_level_t *level)
{
    memcpy(dst, ATLAS_COMMON_OPUS_STREAM_FRAME_MAGIC, 4);
    dst[4] = 1u;                                      // version
    dst[5] = ATLAS_COMMON_OPUS_STREAM_BINARY_HEADER_BYTES;   // header length
    dst[6] = 0u;                                      // flags
    dst[7] = 1u;                                      // channels
...
{
    if (duration_ms < ATLAS_COMMON_OPUS_FRAME_MS) {
        return ATLAS_COMMON_OPUS_FRAME_MS;
    }
...
static uint32_t clamp_stream_duration(uint32_t duration_ms)
{
    ...
    if (duration_ms > ATLAS_COMMON_OPUS_STREAM_MAX_DURATION_MS) {
        return ATLAS_COMMON_OPUS_STREAM_MAX_DURATION_MS;
    }
    return duration_ms;
}
...
static void opus_stream_task(void *arg)
{
    ...
    wire = (uint8_t *)heap_caps_malloc((size_t)outbuf_size + ATLAS_COMMON_OPUS_STREAM_BINARY_HEADER_BYTES, MALLOC_CAP_8BIT);
    ...
    esp_websocket_client_config_t ws_cfg = {
        ...
        .task_name = s_backend.websocket_task_name == NULL ? "atlas_ws_audio" : s_backend.websocket_task_name,
        .task_stack = 6144,
        .task_prio = 4,
    };
    ...
    while (!s_stream_stop_requested &&
           ...
           stream_now_ms() - connect_started < ATLAS_COMMON_OPUS_STREAM_CONNECT_TIMEOUT_MS) {
    ...
    snprintf(start_json,
             sizeof(start_json),
             "{\"type\":\"start\",\"protocol\":\"" ATLAS_COMMON_OPUS_STREAM_PROTOCOL "\","
             "\"device_id\":\"%s\",\"codec\":\"opus\",\"sample_rate\":%u,"
             "\"channels\":1,\"frame_ms\":%u,\"header\":\"AOP1\",\"duration_ms\":%" PRIu32 "}",
             s_backend.device_id == NULL ? "device" : s_backend.device_id,
             (unsigned)ATLAS_COMMON_OPUS_SAMPLE_RATE,
             (unsigned)ATLAS_COMMON_OPUS_FRAME_MS,
             duration_ms);
    if (esp_websocket_client_send_text(client, start_json, strlen(start_json), pdMS_TO_TICKS(ATLAS_COMMON_OPUS_STREAM_SEND_TIMEOUT_MS)) < 0) {
        final_err = ESP_FAIL;
    ...
    stream_status_unlock();
    common_note_stage(true, "opus_streaming");
    ...
    while (!s_stream_stop_requested && esp_websocket_client_is_connected(client)) {
        ...
        if (common_is_muted(&mute_remaining_ms)) {
            stream_status_lock();
            ...
            stream_status_unlock();
            const uint32_t delay_ms = mute_remaining_ms > 0u && mute_remaining_ms < ATLAS_COMMON_OPUS_FRAME_MS ?
                                      mute_remaining_ms :
                                      ATLAS_COMMON_OPUS_FRAME_MS;
        ...
        atlas_common_opus_mic_level_t level = {0};
        final_err = common_capture_pcm(pcm, frame_samples, &level);
        if (final_err != ESP_OK) {
            stream_status_lock();
        ...
        write_stream_binary_header(wire,
                                   seq,
                                   stream_now_ms(),
                                   ATLAS_COMMON_OPUS_SAMPLE_RATE,
                                   ATLAS_COMMON_OPUS_FRAME_MS,
                                   (uint16_t)out.encoded_bytes,
                                   &level);
        memcpy(wire + ATLAS_COMMON_OPUS_STREAM_BINARY_HEADER_BYTES, opus, out.encoded_bytes);
        const int wire_len = (int)(ATLAS_COMMON_OPUS_STREAM_BINARY_HEADER_BYTES + out.encoded_bytes);
        const int sent = esp_websocket_client_send_bin(client, (const char *)wire, wire_len, pdMS_TO_TICKS(ATLAS_COMMON_OPUS_STREAM_SEND_TIMEOUT_MS));
    ...
    if (client != NULL && esp_websocket_client_is_connected(client)) {
        char end_json[360];
        atlas_common_opus_stream_status_t snapshot;
        atlas_common_opus_stream_get_status(&snapshot);
        snprintf(end_json,
                 sizeof(end_json),
                 "{\"type\":\"end\",\"protocol\":\"" ATLAS_COMMON_OPUS_STREAM_PROTOCOL "\","
                 "\"device_id\":\"%s\",\"reason\":\"%s\",\"frames\":%" PRIu32 ","
        ...
        (void)esp_websocket_client_send_text(client, end_json, strlen(end_json), pdMS_TO_TICKS(ATLAS_COMMON_OPUS_STREAM_SEND_TIMEOUT_MS));
    ...
    stream_status_unlock();
    common_note_stage(final_err == ESP_OK ? true : false,
...
esp_err_t atlas_common_opus_stream_start(const atlas_common_opus_stream_config_t *config)
{
    ...
    s_stream_status.frame_ms = ATLAS_COMMON_OPUS_FRAME_MS;
    s_stream_status.sample_rate = ATLAS_COMMON_OPUS_SAMPLE_RATE;
    ...
    BaseType_t ok = xTaskCreateWithCaps(opus_stream_task,
                                        s_backend.stream_task_name == NULL ? "atlas_common_opus_ws" : s_backend.stream_task_name,
                                        ATLAS_COMMON_OPUS_STREAM_TASK_STACK,
                                        NULL,
                                        ATLAS_COMMON_OPUS_STREAM_TASK_PRIORITY,
                                        &s_stream_task,
...
esp_err_t atlas_common_opus_probe_run(uint16_t duration_ms, atlas_common_opus_probe_result_t *result)
{
    ...
    result->frame_ms = ATLAS_COMMON_OPUS_FRAME_MS;
    result->frames_requested = (uint16_t)((result->requested_ms + ATLAS_COMMON_OPUS_FRAME_MS - 1u) /
                                          ATLAS_COMMON_OPUS_FRAME_MS);
    ...
    for (uint16_t i = 0; i < result->frames_requested; ++i) {
        ...
        err = common_capture_pcm(pcm, result->frame_samples, &level);
        if (err != ESP_OK) {
            break;
        }
